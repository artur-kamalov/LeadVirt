...
| `/login`, `/signup`                        | Email OTP AuthFlow                                                  | публичный; Telegram account auth в UI отсутствует |
...
| -------------------------------------- | --------------------------------------------------------------------------------------- |
| `/api/auth/*`, `/api/auth/me`          | password, email OTP, logout, password reset; legacy Telegram auth выключен по умолчанию |
...
### 10.0 Текущий продуктовый контракт

- Публичные `/login` и `/signup` используют только Email OTP. Ошибка или отключение email delivery не переключает пользователя на Telegram account auth.
- Legacy Telegram account auth является отдельной серверной capability: без явного разрешения она выключена и не создает новые сессии.
- Существующие сессии с `authMode: "telegram"` продолжают распознаваться на период управляемой миграции. Это compatibility path, а не доступный способ нового входа.
- Telegram Bot integration для сообщений клиентов не связана с account auth. Подключение бота, webhook ingress, Inbox и outbound delivery остаются LIVE без изменений.
- Production predeploy inventory от 2026-07-19 обнаружил 2 legacy Telegram-authenticated users и 21 session. До удаления compatibility path каждому пользователю нужен verified email mapping или явное retirement-решение; для каждой сессии требуется явное revoke-or-expire решение.
...
- Telegram Login/OIDC остаются legacy server capability, выключенной по умолчанию. UI их не предлагает; API продолжает распознавать уже созданные Telegram sessions до завершения миграции.
...
12. Legacy account-auth inventory еще не закрыт: 2 Telegram-authenticated users и 21 session требуют verified email mapping либо явного retirement/revoke-or-expire решения. Business Telegram channels к этому ограничению не относятся.
