...
- [x] Added Telegram login account switching: API exposes public bot id, `/login` clears the current LeadVirt session, uses Telegram OIDC `prompt=login select_account` for the "different account" action, verifies OIDC `id_token` server-side, and regular Telegram callbacks remain accepted.
- [x] Hardened Telegram account switching against fast OAuth popup close by registering the OIDC message listener before opening Telegram and accepting delayed `auth_result` messages after close.
